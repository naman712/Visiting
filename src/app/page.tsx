...
import { Upload, User, Mail, Building2, Phone, Briefcase, Send, Loader2, CheckCircle2, X, ScanLine, Camera, ImageIcon } from "lucide-react";
    ...
    <div className="max-w-4xl mx-auto px-4 py-6 sm:py-10">
      <div className="mb-6 sm:mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-xl sm:text-2xl font-bold text-slate-800">Scan a Visiting Card</h1>
        </div>
        <p className="text-slate-500 text-sm ml-13">Upload a card image or enter details manually — we&apos;ll send a personalised welcome email.</p>
      </div>

      {/* Tab toggle */}
      <div className="flex gap-1 bg-slate-100 p-1 rounded-lg w-fit mb-6 sm:mb-8">
            ...
            <>
              ...
              <div className="sm:hidden grid grid-cols-2 gap-3">
                <button
                  ...
                  disabled={mode === "ocr"}
                  className="flex flex-col items-center gap-2 py-5 bg-white border-2 border-slate-200 rounded-xl text-slate-600 font-medium text-sm active:bg-slate-50 disabled:opacity-50"
                >
              ...
              {/* Preview (mobile) */}
              {preview && (
                <div className="sm:hidden relative bg-white rounded-xl border border-slate-200 p-3">
                  <div className="relative inline-block w-full">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={preview} alt="Card preview" className="max-h-48 mx-auto rounded-lg object-contain block" />
                    <button
              ...
              {mode === "ocr" && !preview && (
                <div className="sm:hidden flex items-center justify-center gap-2 text-indigo-600 py-2">
                  <Loader2 size={16} className="animate-spin" />
                  <span className="text-sm">Extracting details with Gemini AI…</span>
                </div>
              )}
              ...
              <div
                {...getRootProps()}
                className={`hidden sm:block border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all ${
                  isDragActive
